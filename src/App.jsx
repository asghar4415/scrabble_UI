...
const GameProvider = ({ children }) => {
  // Initial state now includes human_objective
  const [gameState, setGameState] = useState({
        board: Array(15).fill(null).map(() => Array(15).fill(null)),
        scores: { human: 0, ai: 0 },
        current_player: 'human',
        player_rack: [], // Keep player_rack here, GamePage uses it directly
        game_over: false,
        first_move: true,
        tiles_in_bag: 0,
        human_objective: null // Initialize objective
  });
  ...
  const clearNotifications = useCallback(() => {
    ...
    const initGame = useCallback(async () => {
        ...
        // Make sure the received state structure matches the expected one
        setGameState({
            board: state.board || Array(15).fill(null).map(() => Array(15).fill(null)),
            scores: state.scores || { human: 0, ai: 0 },
            current_player: state.current_player || 'human',
            player_rack: state.player_rack || [], // Use player_rack from response
            game_over: state.game_over || false,
            first_move: state.first_move === undefined ? true : state.first_move, // Handle undefined
            tiles_in_bag: state.tiles_in_bag || 0,
            human_objective: state.human_objective || null // Get objective from response
        });
        ...
        // Reset to a defined initial state on error
        setGameState({
            board: Array(15).fill(null).map(() => Array(15).fill(null)),
            scores: { human: 0, ai: 0 },
            current_player: 'human',
  ...
  const startNewGame = useCallback(async () => {
   ...
   const handleStateUpdate = useCallback((newState) => {
    ...
    setGameState({
        ...
        player_rack: newState.player_rack, // Update rack from new state
        game_over: newState.game_over,
        first_move: newState.first_move,
        tiles_in_bag: newState.tiles_in_bag,
        human_objective: newState.human_objective // Update objective
    });
    ...
    // Check if game over message needs appending (if backend didn't include it)
  ...
  const getWordInfoForAPI = useCallback(() => {
    ...
    while (startPos > 0) {
        ...
        if (gameState.board[checkR]?.[checkC]) { startPos--; } else { break; }
    }
    ...
    let endPos = lastTile.position[axis];
     while (endPos < 14) {
        ...
        if (gameState.board[checkR]?.[checkC]) { endPos++; }
        else {
            const nextPosIsPlaced = placedTiles.some(t => t.position[axis] === endPos + 1);
            if (nextPosIsPlaced) { endPos++; } else { break; }
        }
    }
    let word = '';
    ...
    for (let i = startPos; i <= endPos; i++) {
        ...
        if (placedLetter) { word += placedLetter; }
        else if (boardLetter) { word += boardLetter; }
        else {
  ...
  const makePlayerMove = useCallback(async () => {
    clearNotifications();
    if (placedTiles.length === 0) { setError("Place at least one tile to make a move."); return; }
    if (gameState?.first_move && !checkFirstMoveCenter(placedTiles, direction)) { setError("First move must cross the center square (H8)."); return; }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/game/move`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(wordInfo),
      });
      const newState = await response.json();
      if (!response.ok) { throw new Error(newState.detail || `Move rejected by server (HTTP ${response.status})`); }
  ...
  }, [placedTiles, gameState, direction, checkFirstMoveCenter, getWordInfoForAPI, handleStateUpdate, clearNotifications, setError]); // Added setError


  const passTurn = useCallback(async () => {
    clearNotifications();
    if(placedTiles.length > 0) { if(!window.confirm("You have tiles placed on the board. Passing will clear them. Continue?")) { return; } }
    setIsAiThinking(true);
    resetTurnState(); // Reset turn state *before* making the API call
    try {
      const response = await fetch(`${API_BASE_URL}/api/game/pass`, { method: 'POST' });
      const newState = await response.json();
      if (!response.ok) { throw new Error(newState.detail || `Failed to pass turn (HTTP ${response.status})`); }
      handleStateUpdate(newState);
    } catch (err) {
  ...
  }, [clearNotifications, handleStateUpdate, placedTiles.length, resetTurnState, setError]); // Added setError dependency
  ...
  const validatePlacement = useCallback((row, col) => {
      ...
      if (placedTiles.length === 1) {
          if (row === firstTilePos.row && col !== firstTilePos.col) { currentDirection = 'horizontal'; }
          else if (col === firstTilePos.col && row !== firstTilePos.row) { currentDirection = 'vertical'; }
          else if (row === firstTilePos.row && col === firstTilePos.col) { return { valid: false }; }
          else { setError("Tiles must be placed in a straight line (horizontally or vertically)."); return { valid: false }; }
      }
      ...
      if (allTilesIncludingNew.some(t => t.position[fixedAxis] !== fixedValue)) { setError("Tiles must all be on the same row (horizontal) or column (vertical)."); return { valid: false }; }
      if (row !== fixedValue && currentDirection === 'horizontal') { setError("Tiles must be placed on the same row for a horizontal word."); return { valid: false }; }
      if (col !== fixedValue && currentDirection === 'vertical') { setError("Tiles must be placed on the same column for a vertical word."); return { valid: false }; }
      ...
      for (let i = 0; i < positions.length - 1; i++) {
          if (positions[i+1] - positions[i] > 1) {
              ...
              for (let p = positions[i] + 1; p < positions[i+1]; p++) {
                  ...
                  if (!gameState.board[rCheck]?.[cCheck]) { gapFilledByBoard = false; break; }
              }
              if (!gapFilledByBoard) { setError("Tiles must be placed adjacent to each other or existing tiles without gaps."); return { valid: false }; }
          }
      }
  ...
  const placeTileOnBoard = useCallback((row, col) => {
    clearNotifications();
    if (!selectedTile) { setError("Select a tile from your rack first."); return; }
    if (gameState?.current_player !== 'human' || gameState?.game_over) { setError("Cannot place tiles now."); return; }
    if (gameState?.board[row][col] !== null) { setError("Cannot place tile on an already occupied square."); return; }
    if (placedTiles.some(t => t.position.row === row && t.position.col === col)) { setError("Square already contains a tile placed this turn."); return; }
    const placementValidation = validatePlacement(row, col);
    if (!placementValidation.valid) { return; }
    if (placementValidation.newDirection && placementValidation.newDirection !== direction) { setDirection(placementValidation.newDirection); }
    setPlacedTiles(prev => [...prev, { letter: selectedTile.letter, position: { row, col }, rackIndex: selectedTile.index }]);
    setSelectedTile(null);
  ...
  const handleTileClick = useCallback((tile) => {
    ...
    if (isPlaced) {
      ...
      // If recalling the last placed tile, reset direction maybe? Or handle in validatePlacement
      setSelectedTile(null);
    } else {
      if (selectedTile && selectedTile.index === index) { setSelectedTile(null); }
      else { setSelectedTile({ letter: tile.letter, index: tile.index }); }
    }
  }, [selectedTile, gameState, clearNotifications, placedTiles.length]); // placedTiles.length dependency
  ...
  // Context value now includes human_objective
  const value = {
    gameState, // Includes human_objective now
    loading,
    error,
    message,
    selectedTile,
      ...
      <div className="scrabble-board-frame">
        <div className="welcome-content">
          <div className="scrabble-title">
            {['S','C','R','A','B','B','L','E'].map((letter, i) => ( <span key={i}>{letter}</span> ))}
          </div>
          <p className="tagline">The Classic Word Game - Advanced AI</p>
          <div className="game-options">
            <button className="game-option-btn vs-ai" onClick={() => navigate('/vs-ai')} aria-label="Play against the AI">
              <span className="tile-letter">P</span>
              <div className="option-text"> <h2>Play vs AI</h2> <p>Challenge our Advanced Scrabble AI.</p> </div>
            </button>
            <button className="game-option-btn ai-vs-ai" onClick={() => navigate('/ai-vs-ai')} aria-label="Watch AI vs AI (Coming Soon)">
              <span className="tile-letter">A</span>
              <div className="option-text"> <h2>AI vs AI</h2> <p>Watch the algorithms battle it out!</p> </div>
            </button>
          </div>
           <div className="scrabble-rules">
                ...
                <ol>
                    ...
                    <li>Blanks (' ') are wildcards (0 points). Power Tiles ('D') have special effects!</li>
...
const GamePage = () => {
  const game = useGame(); // Access the full context

  const handleCellClick = useCallback((cell) => {
    if(game.isAiThinking) return ;
      game.clearNotifications();
      if (game.selectedTile) { game.placeTileOnBoard(cell.row, cell.col); }
      else { game.setSelectedCell(cell); }
  }, [game]); // game object includes all needed functions and state

  
  // Show loading screen while game state is initializing
  if (!game.gameState || game.gameState.player_rack === undefined || game.loading) {
      return ( <div className="loading-screen"> {game.loading ? 'Connecting to game...' : 'Failed to load game. Please Refresh'} </div> );
  }

  // Prepare rack tiles based on the current gameState
   const availableRackTiles = game.gameState.player_rack ? game.gameState.player_rack.map((letter, index) => ({
        letter,
        index,
      ...
      <div className="game-container">
          {game.error && (
            <div className="error-message">
              {game.error}
              <button className="dismiss-error" onClick={game.clearNotifications} aria-label="Dismiss error">×</button>
            </div>
          )}
           {game.message && !game.error && (
            <div className="info-message">
              {game.message}
               <button className="dismiss-info" onClick={game.clearNotifications} aria-label="Dismiss message">×</button>
            </div>
          )}

          {/* Pass all necessary props to Board, including human_objective */}
          <Board
            ...
            humanObjective={game.gameState.human_objective} // Pass objective down
    ...
    <div className="coming-soon">
      <div className="scrabble-board-frame">
          <div className="welcome-content">
            ...
            <button onClick={() => navigate('/')} className="control-button new-game-btn" style={{marginTop: '20px'}}> Go Back Home </button>
          </div>
       </div>
    </div>
  );
      ...
      <GameProvider>
         <div className="app">
            <Routes>
              ...
              <Route path="*" element={ <div style={{ textAlign: 'center', marginTop: '50px' }}> <h1>404 - Page Not Found</h1> <p>Sorry, the page you are looking for does not exist.</p> </div> } />
            </Routes>
         </div>
      </GameProvider>
    </Router>
